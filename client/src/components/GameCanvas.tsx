...
import { DODGE_COOLDOWN_SECONDS, DODGE_INVULNERABILITY_SECONDS, DODGE_PERFECT_WINDOW_SECONDS, IDLE_NEEDLE_WAIT_SECONDS, selectSoundEventsForPlayback } from "@/game/rules";
...
const tutorialSteps = [
  ...
  { title: "危険から離れる", copy: "橙の線は突進、紫赤の輪は波動、黄橙の地面マークは砲撃です。予告範囲から離れてください。完全に1秒止まると、停止針の予告後に落下針が来ます。" },
  { title: "回避を使う", copy: `回避は${DODGE_INVULNERABILITY_SECONDS}秒の無敵時間つきです。再使用まで${DODGE_COOLDOWN_SECONDS}秒あるため、危険な攻撃に合わせて使います。予告終了前${DODGE_PERFECT_WINDOW_SECONDS}秒に危険範囲から出るか、無敵中に攻撃を受けると完全回避になり、短時間攻撃力が上がります。ボスの行動も中断できます。` },
  { title: "装備を組み立てる", copy: "攻撃は初期6枠、補助は4枠です。無限モードでは10レベルごとに攻撃枠が増え、最大14枠になります。対応する2つの攻撃を両方レベル3にすると、自動で1つの進化武器になります。" },
];
...
export default function GameCanvas() {
  ...
  return (
    <main ref={mainRef} className={`game-shell viewport-${viewportMode} ${isPaused ? "is-paused" : ""} ${healthPercent <= 25 ? "low-health" : ""}`} onContextMenu={(event) => event.preventDefault()} style={{ "--stick-opacity": playerSettings.stickOpacity } as CSSProperties} aria-label="サバサバ" data-testid="game-shell" data-phase={snapshot.phase} data-mode={currentMode}>
      ...
      <EvolutionNotice weapons={evolvedWeapons} visible={runStarted && snapshot.phase === "playing" && !isPaused && countdownRemaining === 0} />
